...
| Learned matrix-level spike variance | Replace fixed $\kappa$ by one absolute $\xi_k$ per matrix with an exact M-step. | Best full-KL $97.66\%$, final $97.61\%$, KL/example $0.4615$; more aggressive and less accurate than fixed $\kappa$. |
